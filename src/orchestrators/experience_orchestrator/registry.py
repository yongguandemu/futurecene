"""registry.py — 游戏经验学习调度官内部能力注册表（规格书 5.2）

能力条目（单实现，全部由 orchestrator.handle 分发）：
- experience:start / stop / decide / feedback / inject_task / stats / knowledge / plan
实际执行经 bind(orchestrator.handle) 绑定，resolve 返回已绑定的真实调度函数。

# 模块内容清单（8 项契约）
1. 模块身份标识：experience 调度官 · registry · 能力注册表（experience:start 等 8 项）
...
HANDLERS = {
    "experience:start": [object],
    "experience:stop": [object],
    "experience:decide": [object],
    "experience:feedback": [object],
    "experience:inject_task": [object],
    "experience:stats": [object],
    "experience:knowledge": [object],
    "experience:plan": [object],
}
