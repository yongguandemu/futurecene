"""registry.py — LLM 调度官内部能力注册表（规格书 5.2）

能力条目：
- llm:chat：OpenAIClient（主）→ GLMClient（降级备选）
- llm:stream_chat：同上

降级链顺序即列表顺序（索引 0 为主，其余为备）。
说明：llm:active_dialogue 为内部子能力，经 orchestrator.handle 直接分发
（不列入对外能力清单，避免与规格书/测试约定的 llm:chat、llm:stream_chat 冲突）。
实际执行经 bind(orchestrator.handle) 绑定，resolve 返回已绑定的真实调度函数。

# 模块内容清单（8 项契约）
1. 模块身份标识：llm 调度官 · registry · 能力注册表
...
5. 依赖声明：glm_client、openai_client、src.shared.capability_registry（CapabilityRegistry / UnknownEngine）
...
HANDLERS = {
    "llm:chat": [OpenAIClient, GLMClient],
    "llm:stream_chat": [OpenAIClient, GLMClient],
}
