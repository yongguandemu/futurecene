"""registry.py — 安全调度官内部能力注册表（规格书 5.2）
...
# 模块内容清单（8 项契约）
1. 模块身份标识：safety · registry · 能力注册表（safety:check_input/check_output/reload_rules）
2. 配置契约：无
...
HANDLERS = {
    "safety:check_input": [object],    # 实际经 orchestrator.handle 分发
    "safety:check_output": [object],
    "safety:reload_rules": [object],
}
